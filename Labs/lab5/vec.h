...
//  lab5
//
//  Created by Sriyuth Sagi on 2/14/18.
...
public:
    ...
    void print() const {cout << m_alloc << endl; cout << m_size << endl; cout << m_data << endl;}
    
    size_type get_m_alloc() const {return m_alloc;}
...
template <class T> int remove_matching_elements(Vec<T>& set, T value) {
    int num=0;
    int i=0;
    while (i<set.size()) {
        if (set[i] == value) {
    ...
    if (set.size() < set.get_m_alloc()/2) {
        Vec<T> copy(set);
        set.clear();
        set.resize(copy.size());
