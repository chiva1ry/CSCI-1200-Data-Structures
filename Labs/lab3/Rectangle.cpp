...
: m_lower_left(lower_left), m_upper_right(upper_right)
{
    for (int x=lower_left.x();x<=upper_right.x();x++) {
        for (int y=lower_left.y();y<=upper_right.y();y++) {
            m_points_contained.push_back(Point2D(x, y));
        }
    }
}
...
bool Rectangle::add_point(const Point2D& point) {
    for (int i=0;i<m_points_contained.size();i++) {
        if (point.x() == m_points_contained[i].x() && point.y() == m_points_contained[i].y()) {
            return false;
        }
    }
    if (is_point_within(point)) {
        m_points_contained.push_back(point);
        return true;
    }
    return false;
}
