...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y) {
    ...
    for (int y=0; y<blocked_grid[0].size(); ++y) {
        for (int x=0; x<blocked_grid.size(); ++x) {
            bool check = true;
            for (int i=0; i<validPaths[0].size(); i++) {
                if (x == validPaths[0][i].x && y == validPaths[0][i].y) {
                    cout << " $";
                    check = false;
                }
            }
    ...
    for (int i = 0; i < validPaths.size(); i++) {
        cout << i << ": " << endl;
        for (int j = 0; j < validPaths[i].size(); j++) {
            cout << "(" << validPaths[i][j].x << ", " << validPaths[i][j].y << ") ";
        }
        cout << endl;
    }
}
...
int numberPaths(int x, int y, const vector<vector<bool> >& blocked, vector<Point> path = vector<Point>()) {
    if (x == 0 && y == 0) {
    ...
    if (x == 0 && y > 0) {
        return numberPaths(x, y-1, blocked, path);
    } else if (y == 0 && x > 0) {
        return numberPaths(x-1, y, blocked, path);
    }
    return numberPaths(x-1, y, blocked, path) + numberPaths(x, y-1, blocked, path);
}
...
int main(int argc, char* argv[]) {
    ...
    cout << numberPaths(start_x, start_y, blocked_grid) << endl;
    print_grid(blocked_grid, start_x, start_y);

    // Start here with your code...


    cout << validPaths[0][1].x << endl;
