...
int main() {
    ...
    std::cout << "Smaller Numbers: " << std::flush;
    for (int i=0;i<length;i++) {
    ...
    float root_1_array[5];
    float root_2_array[5];
    float smaller_root[5];
    for (int i=0;i<5;i++) {
        bool success = find_roots(my_a,b_array[i],c_array[i], root_1_array[i],root_2_array[i]);
        smaller_root[i] = root_2_array[i];
        std::cout << smaller_root[i] << std::endl;
    }
    */
